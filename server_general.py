                ...
                try:
                    ...
                    if packet_structure.get('start_bit') == 'EEEE' and packet_structure.get('stop_bit') == 'AAAA':
                        ...
                        if packet_structure.get('protocol_number') == '01':
                            ...
                            imei, device_id = decoder.login_packet_decoder(received)
                            ...
                            print(f"[SERVER] [{self.addr[0]}] [{datetime.now()}] Valid connection from: {device_id}")
                            
                            # Achknowledge login and send responce to device
                            response_packet = decoder.acknowledgement_packet_encoder(self.server_id, device_id)
                            response_packet += b"\n\r"
